...
class Assignments extends React.Component {
  ...
  onEducationItemChanged = (event, index, datePickerName) => {
    let name;
    let value;
    ...
    let edu = this.state.education.map((next, i) => {
      if (index === i) {
        return {...next, [name]: value}
      }
      return next
    })
    this.onValueChanged('education', edu)
  }

  onEducationDateChanged = (event, index, datePickerName) => {
    let name;
    let value;
    if (datePickerName === 'startDate' || datePickerName === 'endDate') {
      name = datePickerName
  ...
  render () {
    ...
    return (
      <Fragment>
          ...
          <Grid container spacing={2} direction="row" justify={"space-between"}>
            <Grid item xs={4}>
              <TextField id="outlined-basic" label={'First Name'} variant="outlined"
                         name='firstName' onChange={this.onTextChanged} style={{width: '100%'}}/>
            </Grid>
            <Grid item xs={4}>
              <TextField id="outlined-basic" label={'Last Name'} variant="outlined"
                         name="lastName" onChange={this.onTextChanged} style={{width: '100%'}}/>
            </Grid>
            <Grid item xs={3} sm={3}>
              <DatePicker
                fullWidth
            ...
            {this.state.education.map((el, index) => {
              return (
                <Education
                  ...
                  index={index}
                  testChange={this.onEducationItemChanged}
        ...
        {this.state.cvPreview && <CvPreview state={this.state}/>}
      </Fragment>
    )
  }
}
